        ...
        switch (tok->type) { 
            ...
            case IF: {
                ...
                if (cond_value) {
                    ...
                    //return IMaster::MasterRun(getChild(tree, 1), param);
                } else {
                    if (k <= 2) {
                        return cond_value;
                    } else {
                        ...
                        //return IMaster::MasterRun(getChild(tree, 2), param);
                    }
                }
                return cond_value;
            }
            ...
            case DEF: {
                int k = tree->getChildCount(tree);
                for (int i = 0; i < k; i++) {
                    pANTLR3_BASE_TREE temp = getChild(tree, i);
                    string var(getText(getChild(tree, i)));
                    param->setVal(var);
                    int cnt = temp->getChildCount(temp);
                    if (cnt > 0) {
                        int val = IMaster::MasterRun(getChild(temp, 0), param);
                        param->setVal(var, val);
                    }
                }
                return 0;
            ...
            case PRINT: {
                ...
                for (int i = 0; i < k; i++) {
                    ret = IMaster::MasterRun(getChild(tree, i), param);
                    cout << ret;
                }
                cout << '\n'; 
                return k;
